...
```ts
import { Prisma, type User } from "@openvz/db";
```
...
`DATABASE_URL` comes from the **repo-root `.env`**, loaded by `@openvz/env` — see
[`docs/environment.md`](../../docs/environment.md). `src/client.ts` imports
`@openvz/env/load` before reading it, and `prisma.config.ts` does the same so the
...
- **Auth models are generated.** `User`, `Session`, `Account`, `Verification`
  ...
  the Better Auth config in `@openvz/auth` and re-run `bun run auth:generate`.
