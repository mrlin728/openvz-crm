...
const CONFIG: TrackingConfig = {
	...
	cookieDays: 395,
	hosts: [
		{ host: "openvzai.com", scope: "SITE_AND_SUBDOMAINS" },
...
describe("the allow-list", () => {
	test("covers subdomains only where the scope says so", () => {
		expect(hostAllowed("openvzai.com", CONFIG)).toBe(true);
		expect(hostAllowed("docs.openvzai.com", CONFIG)).toBe(true);
	...
	test("never matches a host that merely ends the same way", () => {
		expect(hostAllowed("notopenvzai.com", CONFIG)).toBe(false);
		expect(hostAllowed("openvzai.com.evil.com", CONFIG)).toBe(false);
	});
...
describe("the domain an event belongs to", () => {
	test("is the parent when the scope covers subdomains", () => {
		expect(matchedHost("docs.openvzai.com", CONFIG)?.host).toBe("openvzai.com");
		expect(matchedHost("openvzai.com", CONFIG)?.host).toBe("openvzai.com");
	});
	...
	test("prefers the host's own row to the parent that also covers it", () => {
		const both: TrackingConfig = {
			...
			hosts: [
				{ host: "openvzai.com", scope: "SITE_AND_SUBDOMAINS" },
				{ host: "docs.openvzai.com", scope: "EXACT_HOST" },
			],
		};

		expect(matchedHost("docs.openvzai.com", both)?.host).toBe(
			"docs.openvzai.com",
		);
		expect(matchedHost("blog.openvzai.com", both)?.host).toBe("openvzai.com");
	});
});
...
describe("the origin check", () => {
	test("accepts an allowed origin and refuses the rest", () => {
		expect(originAllowed("https://docs.openvzai.com", CONFIG)).toBe(true);
...
describe("the submission dedupe key", () => {
	test("collapses the same form inside one minute", () => {
		...
		const parts = { host: "openvzai.com", path: "/pricing", email: "a@b.com" };
	...
	test("separates a different address on the same page", () => {
		...
		expect(
			dedupeKey({ host: "openvzai.com", path: "/p", email: "a@b.com", at }),
		).not.toBe(
			dedupeKey({ host: "openvzai.com", path: "/p", email: "c@d.com", at }),
		);
	});
});
