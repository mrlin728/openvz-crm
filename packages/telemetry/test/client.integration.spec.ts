...
import { posthogHost } from "../src/project";

const real = {
	fetch: globalThis.fetch,
	nodeEnv: process.env.NODE_ENV,
	disabled: process.env.OPENVZ_TELEMETRY_DISABLED,
	enabled: process.env.OPENVZ_TELEMETRY_ENABLED,
	posthogKey: process.env.OPENVZ_POSTHOG_KEY,
...
beforeEach(() => {
	...
	process.env.OPENVZ_TELEMETRY_ENABLED = "1";
	process.env.OPENVZ_POSTHOG_KEY = "phc_test";
	delete process.env.OPENVZ_TELEMETRY_DISABLED;
...
afterEach(() => {
	...
	restore("OPENVZ_TELEMETRY_DISABLED", real.disabled);
	restore("OPENVZ_TELEMETRY_ENABLED", real.enabled);
	restore("OPENVZ_POSTHOG_KEY", real.posthogKey);
...
describe("capture", () => {
	it("sends one event with the install UUID as the only identity", async () => {
		...
		expect(calls[0]?.url.startsWith(posthogHost())).toBe(true);
	...
	it("makes no network call at all when OPENVZ_TELEMETRY_DISABLED is set", async () => {
		process.env.OPENVZ_TELEMETRY_DISABLED = "1";
		resetTelemetryClient();
