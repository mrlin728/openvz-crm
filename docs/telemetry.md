...
**Telemetry is off.** This install sends nothing until an operator sets
`OPENVZ_TELEMETRY_ENABLED="1"` in the root `.env` *and* gives it somewhere to send with
`OPENVZ_POSTHOG_KEY`. Without a key no client is constructed, so every capture is a
no-op even with the flag on. `OPENVZ_TELEMETRY_DISABLED="1"` and `DO_NOT_TRACK=1` still
work and beat the opt-in, so a machine that has said no stays no.

This is the reverse of the usual arrangement, deliberately. The upstream project this
codebase derives from shipped an analytics key pointing at its own host, which meant
every clone reported to a vendor by default. That key is gone. Software people run on
their own hardware should not phone anywhere until they say so.

**Where it goes** is yours to set: `OPENVZ_POSTHOG_KEY` and `OPENVZ_POSTHOG_HOST`, read at
runtime in `packages/telemetry/src/project.ts`. A `phc_` key is write-only — it sends
events and reads nothing back — so it is a public identifier, not a secret. Nothing is
sent while `NODE_ENV` is `test`, so a test run cannot emit.
...
- **Server side only, in everything you run.** `posthog-node` in `apps/api` and `apps/agent`.
  ...
  single exception is the marketing site at `crm.openvzai.com`, which is a web page and not an
...
- **Nothing is sent from a test run.** `NODE_ENV=test` disables it, and `apps/api` and `apps/agent`
  additionally set `OPENVZ_TELEMETRY_DISABLED=1` in their `test` scripts — because a spec is allowed
...
| `tracking_capped` | Submissions the hourly contact cap refused. Matched against `CONTACT_CAP_REASON` (`@openvz/db/tracking`) exactly, so the wording is a constant two files share rather than prose one of them may reword |
...
`crm.openvzai.com` is the one place `posthog-js` runs: web analytics and session replay of strangers
...
`window.location.hostname` against two literals — `crm.openvzai.com` and `www.openvzai.com` — and
