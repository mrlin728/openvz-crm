...
id `WORKSPACE_ID` (the literal `workspace`, in `@openvz/db`, re-exported by `@openvz/auth`
...
- **Permissions come from `@openvz/auth`** — `canRenameWorkspace`, `canChangeRole`,
...
- **Onboarded state is `onboardedAt` inside the plugin's `metadata` blob**, not a
  column; `isOnboarded`/`markOnboarded` (`@openvz/db/workspace`) are the only accessors,
...
- **The slug is the plugin's column**, written by `workspaceSlug(name)`
  (`@openvz/db/workspace`) on rename and create. **Never derive it on read.**
...
- **Signing in with an IdP does not cost you Gmail.** `needsMailboxGrant` (`@openvz/auth`)
