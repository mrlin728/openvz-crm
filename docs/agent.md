...
Default `zai/glm-5.2-fast`; `DEFAULT_AGENT_MODEL` in `@openvz/db/settings` because the
...
**`@openvz/db/blob`** — writers are `lib/brand-images.ts`, `lib/portrait.ts`,
`FaviconService`, `ImageMirrorService`, `prisma/seed.ts`.

- **The key hashes the bytes** — idempotent, and a redesigned mark gets a new URL.
- **`COMPANY_IMAGE_FIELDS` (`@openvz/db/images`) is the one list of picture columns.**
- **Fetch through `@openvz/db/safe-fetch`** — vendor URLs are SSRF vectors.
- **No `BLOB_READ_WRITE_TOKEN` means no photographs**; logos keep the origin URL.
- **`isOptimizable` (`@openvz/db/images`) is the whole rule**: `next.config.ts`
...
`schedules/dispatch.ts`, split by `DIRECT_KINDS` in `@openvz/db/agent-tasks`.
...
- **The trigger is signing in**, via `databaseHooks.session.create.after` in
  ...
  (`@openvz/auth` must not import a Nest provider). Five-minute stand-down, and `auto()`
...
- **Tiny, enforced by the write path** — `MAX_NARRATIVE` (320) and `MAX_LINE` in
  `@openvz/db/workspace`. It is prompt-cached and precedes every question.
...
- **Actions are structured permissions.** `crm.activity.create` separately names
  ...
  for replay safety.
- **CRM events are shared domain contracts.** `@openvz/db/crm-events` owns the event
