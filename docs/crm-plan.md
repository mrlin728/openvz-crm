...
| Forms | `react-hook-form` + `zodResolver` + `@openvz/ui` Field primitives | §11 — matches mvp exactly. |
...
The `@openvz/ui` `attachment.tsx` component already models the
...
Ported from `an earlier internal prototype's data-table components`, rewriting
`@openvzai/ui` → `@openvz/ui`:
...
Everything it needs already exists in `@openvz/ui`: `table`, `dropdown-menu`,
...
3. **Date-range facets.** mvp's `DataTable` only has single-select radio
   ...
   Last 7 days, This month, Last quarter, Custom) over `@openvz/ui`'s existing
...
5. **Standard cell renderers**, so every list looks like one product:
   ...
   `@openvz/ui`'s existing `empty-cell` rendering `--` for every null — matching
   HubSpot exactly.
...
`react-hook-form` + `zodResolver` + `@openvz/ui` `Field` primitives
