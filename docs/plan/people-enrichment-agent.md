...
4. **A separate Vercel app, sharing `@openvz/db`.** eve is its own runtime with its
...
| CRM access | `@openvz/db` directly | Already a workspace package. A second internal API would be plumbing with no reader. |
...
│   │   └── crm.ts        # @openvz/db queries the tools call
...
| `update_contact` | `@openvz/db` | The only write. §6. |
...
| **1 — Skeleton** | `apps/agent`, `eve link`, `update_contact` writing to `@openvz/db`, schedule claiming a batch, no LinkedIn | A contact's title changes because the agent changed it, on a cron, in a deployed environment. The loop is proven before the vendor is in it. |
