...
Adrian Fenwick is the CEO and co-founder of OPENVZ AI. He previously led
growth at Fleetio and spent four years at Deloitte in risk advisory.

Current role    CEO & Co-founder · OPENVZ AI · 2 yrs 3 mos
...
- **Watch it work.** `actions.requested` / `action.result` render as a plain
  ...
  rejected `github.com/adrian`*. The rejections are the most trust-building thing
  on the page.
...
**`packages/db` and `packages/auth` imported `@openvz/env` without declaring it.**
