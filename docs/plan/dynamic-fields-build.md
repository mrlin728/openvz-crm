...
## 3. `@openvz/db/fields` — the shared core
...
`@openvz/db/currency` and `@openvz/db/workspace`, which are the precedents.
...
The API is one caller of `@openvz/db/fields`; the agent is the other. Add
...
`@openvz/db/fields`, and every tool is a thin call to the same module. An MCP server
...
2. **`@openvz/db/fields`.** Types, key derivation, value schemas, read/write. Unit
...
## 11. Open — decide with the owner, do not guess
