...
NestJS API for the CRM. Runs on Bun, backed by `@openvz/db` and `@openvz/auth`.
...
Bun is the runtime, not just the package manager: `@openvz/db` and `@openvz/auth`
...
straight from Postgres via `@openvz/auth` and calls the routes above with
`credentials: "include"`.
...
Auth options and endpoint hooks require `hooks: {}` (both set in `@openvz/auth`);
