...
describe("externalParticipants", () => {
	const options = {
		ourDomains: new Set(["openvzai.com"]),
		ourAddresses: new Set(["adrian@openvzai.com"]),
	...
	it("keeps only the other side of the conversation", () => {
		const result = externalParticipants(
			[
				person("adrian@openvzai.com"),
				person("colleague@openvzai.com"),
	...
	it("returns nothing for a wholly internal thread", () => {
		expect(
			externalParticipants([person("colleague@openvzai.com")], options),
		).toEqual([]);
	});
	...
	it("leaves nothing to file when the deleted contact is the only outsider", () => {
		expect(
			externalParticipants(
				[person("adrian@openvzai.com"), person("deleted@acme.com")],
				options,
			),
		).toEqual([]);
	});
