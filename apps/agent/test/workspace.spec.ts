...
const profile: WorkspaceProfile = {
	website: "openvzai.com",
	narrative:
		"OPENVZ AI takes a startup from nothing to a SOC 2 or ISO 27001 audit by automating the evidence collection, and sells the platform on an annual subscription.",
	sections: {
...
describe("who we are", () => {
	...
	it("names us, and refuses to invent the rest", () => {
		...
		expect(markdown).toContain("OPENVZ AI");
		expect(markdown).toContain("openvzai.com");
...
describe("a profile belongs to the website it was read from", () => {
	it("is ours while the website is unchanged", () => {
		expect(profileOf(profile, "openvzai.com")).toBe(profile);
	});
...
describe("the website has to be somewhere a fetch can go", () => {
	it("takes a bare domain and gives back a URL", () => {
		expect(websiteUrl("openvzai.com")).toBe("https://openvzai.com");
		expect(websiteUrl(" WWW.Openvzai.com/ ")).toBe("https://www.openvzai.com");
	});

	it("keeps a scheme it can fetch, and a path that means something", () => {
		expect(websiteUrl("http://openvzai.com")).toBe("http://openvzai.com");
		expect(websiteUrl("https://openvzai.com/uk/")).toBe(
			"https://openvzai.com/uk",
		);
	});
