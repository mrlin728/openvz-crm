...
async function main() {
	...
	if (!JOINS_FOR_REAL) {
		console.log(
			"NOTE  OPENVZ AI joins no channel in this run. A join is permanent.",
		);
	}
	...
	if (!publicUnjoined) {
		record(
			...
			true,
			"skipped: OPENVZ AI is already in every public channel",
		);
	...
	if (!privateUnjoined) {
		record(
			...
			true,
			"skipped: no private channel is visible to OPENVZ AI",
		);
	} else if (!JOINS_FOR_REAL) {
	...
	record(
		...
		canInvite,
		canInvite
			? "a user token lets OPENVZ AI add itself to a private channel"
			: "no user token, so OPENVZ AI cannot add itself to a private channel",
	);
