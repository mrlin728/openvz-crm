...
function describe(channel: PickerChannel, canInviteItself: boolean): string {
	...
	if (channel.isMember) return `OPENVZ AI is in${people}`;
	if (!channel.classified) return `Not read from Slack yet${people}`;
	if (!channel.isPrivate) return `OPENVZ AI can join this one${people}`;
	if (canInviteItself) return `Private. OPENVZ AI joins as you${people}`;
	if (channel.inviteRequestedAt)
		return `Private. Waiting on an invite${people}`;
	return `Private. Someone inside has to invite OPENVZ AI${people}`;
}
