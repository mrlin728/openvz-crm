...
A client component must never import a server package. `@openvz/auth` and `@openvz/db`
...
```tsx
"use client";
import { describeSlackScopes, SLACK_SCOPE_GROUPS } from "@openvz/auth";
...
- A `"use client"` file may import from `@openvz/ui`, the tRPC client, and React.
...
Conventional Commits, because the release tooling reads them to decide the next
version and to write the changelog.

```
git commit -m "fix(api): resolve auth token expiry"
```
